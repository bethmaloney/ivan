 ...
 * error. Renaming a method here or there produces a game that is quietly
 * missing a feature -- the failure class tools/web/README.md keeps warning
 * about, and the one the corpora cannot see because a headless replay makes no
 * sound.
 *
 * test/bridge.test.ts parses the EM_JS blocks out of the C++ and checks them
 * against this, in both directions: a call the C++ makes that is not declared
 * here fails, and a declaration nothing calls any more fails too.
 *
