...
(function () {
  ...
  var Enabled = Query.get('sfx') !== 'off';
  var Base = Query.get('sfxbase');
  ...
  var MaxLatencyMs = 250;

  var Ctx = null;
  var Master = null;
  var Buffers = new Map();   /* url -> AudioBuffer, or a Promise while in flight,
                                or null once it has failed and been reported */
  var Voices = 0;
  var Log = [];
  var Counts = { played: 0, dropped: 0, failed: 0 };
  ...
  /* ---- the audio context ------------------------------------------------
     ...
     are registered here, once, for the context this file owns. */
  ...
  function Load(Path) {
    var Cached = Buffers.get(Path);

    if(Cached !== undefined)
      return Cached;

    var Pending = fetch(Resolve(Path))
      ...
      .catch(function (Error) {
        ...
        Counts.failed++;
        console.warn('ivan: no sound for ' + Path + ' (' + Error.message + ')');
        return null;
      });
  ...
  function Start(Buffer, Volume) {
    if(!Buffer || Voices >= MaxVoices) {
      Counts.dropped++;
      return;
    }
  ...
  function Play(Path, Volume) {
    ...
    var Requested = performance.now();

    Promise.resolve(Load(Path)).then(function (Ready) {
      if(!Ready)
        return;
  ...
  globalThis.ivanSfx = {
    ...
    played: function () { return Log.slice(); },
